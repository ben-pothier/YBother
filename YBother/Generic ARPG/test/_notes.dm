/*
inventory/shop click

allow selling at shop

inventory a little larger or bags to make it bigger
...
new skill for each class

new crafting for bowyer and arcane dynamo

get new items into the game -> boots, bracers, gauntlets, cloak

mini map

combat needs to be better balanced

better explanation about bows

enemies respawn in starting location, not death location

player respawn options - (here (penalty), area start (minor penalty), last visited town (no penalty))

better inventory GUI

have player customize full skill tree, personality, one 'legendary' item (perhaps some mod options, or mod list with a number of points) and create a story in which this is all taken away
and needs to be earned back... personality and certain things (race, gender, etc) may change story options)

very long convoluted quest chain(s) that result in literally unique equipment (there is only one, may permanently change map during the quest chains)
*/
