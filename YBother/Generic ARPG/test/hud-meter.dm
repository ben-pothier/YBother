...
XPMeter
	...
	New(mob/m)
		..(m, "experience", "experience_needed")
		// create the label
		var/obj/o = add(0, 0, "")
		o.overlays += hud_label("<text align=right>EXP", layer = layer, pixel_y = 5)

		pos(8, Constants.VIEW_HEIGHT * Constants.ICON_HEIGHT - 72)
...
mob
	...
	init_hud()
		...
		if(client && Constants.USE_MANA_METER)
			mana_meter = new(src)
		if(client && Constants.USE_XP_METER)
			xp_meter = new(src)
