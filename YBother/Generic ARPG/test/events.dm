...
mob
	proc
		...
		no_melee_equipment(Ability/ability)
			src << "You can't use [ability.name] because you do not have a melee weapon equipped."
		...
		you_joined_party()
			...
			if(party_display)
				party_display.refresh()
		create_party()
			src << "You created a party."
			if(party_display)
				party_display.refresh()
		must_be_party_leader()
			src << "You must be the party leader."
			if(party_display)
				party_display.refresh()
		promoted_to_party_leader()
			src << "You have been promoted to party leader."
			if(party_display)
				party_display.refresh()
		party_is_full()
			src << "The party is full."
			if(party_display)
				party_display.refresh()
