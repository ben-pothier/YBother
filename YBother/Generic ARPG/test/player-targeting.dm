...
mob
	...
	key_down(k)
		...
		else if(k == Constants.KEY_SELF_TARGET)
			hide_show_name()
	...
	proc
		...
		hide_show_name()
			if(src.show_name == 0)
				src.show_name = 1
				src.m_name = src.overlay()
				src.m_name.PixelY(-12)
				src.m_name.PixelX(-48)
				src.m_name.TextBounds(128, 32)
				if(src && src.m_name)
					src.m_name.Text("<text align=center>[src.name]</text>")
			else
				src.show_name = 0
				src.m_name.Del()
