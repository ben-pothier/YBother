...
MagicCombat
	...
	attack(mob/attacker, mob/target, damage)
		...
		if(prob(round(target.resistance*1.25) - attacker.mind))
			target.damage_number("resist")
			return 0

		// magic attacks use the "mind" stat to determine critical hits
		if(attacker && prob(abs(attacker.mind - target.willpower)))
...
PhysicalCombat
	...
	attack(mob/attacker, mob/target, damage)
		...
		if(prob(round(target.agility*1.25) - attacker.dexterity))
			target.damage_number("dodge")
			return 0

		// the attack has a chance to be a critical hit
		if(attacker && prob(abs(attacker.dexterity - target.agility)))
			target.effect("critical-hit")
			damage *= 2
			target.noise('critical-hit.wav')
