	...
	proc
		...
		action(t)
			...
			if(path || destination)
				...
				if(client.focus == src)
					if(client.Mouse == 1)
						if(client.mouse_py > src.py)
							d += NORTH
						else if(client.mouse_py  < src.py)
							d += SOUTH
						if(client.mouse_px < src.px)
							d += WEST
						else if(client.mouse_px > src.px)
							d += EAST
		...
		slow_down()
			...
			if(client)
				...
				if(client.focus != src || (client.Mouse == 0) || (abs(src.px - client.mouse_px) < 2))
					if(vel_x > decel)
						vel_x -= decel
					else if(vel_x < -decel)
						vel_x += decel
				...
				if(client.focus != src || (client.Mouse == 0) || (abs(src.py - client.mouse_py) < 2))
					if(vel_y > decel)
						vel_y -= decel
					else if(vel_y < -decel)
						vel_y += decel
